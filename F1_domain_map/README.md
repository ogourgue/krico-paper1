...
- **CCAMLR Statistical Subareas shown:** 48.1, 48.2, 48.3, 48.4, 48.5, 48.6, 88.3.
...
- **CCAMLR subarea labels:** centered on polygon centroids in framed semi-transparent boxes (style matching panel labels in F4 / F5; the underlying polygon and bathymetry remain visible through the label frame).
- **Two legends:**
  - Upper right, opaque: zone / domain key (3 entries — domain, spawning zone, continental shelf).
  - Lower left, opaque: CCAMLR subarea code → geographic name lookup (7 entries — 48.1 Antarctic Peninsula, 48.2 South Orkney Islands, 48.3 South Georgia, 48.4 South Sandwich Islands, 48.5 Weddell Sea, 48.6 Bouvet Island, 88.3 Amundsen Sea).
